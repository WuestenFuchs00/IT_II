...
int main(){
    ...
    for ( int i = 0; i < MAX; i++ ) {
        ...
        scanf("%f", &((bohnen+i)->gewicht)); // &((*(bohnen+i)).gewicht)
 
        printf("\nBitte geben Sie die Staerke der %i. Bohnensorte ein [0,1]:\n", i+1);
        scanf("%i", &iStaerke);
        (bohnen+i)->staerke = (STAERKE) iStaerke;
    }
 
    //Ausgeben
    printf("\nGewicht / Staerke");
    for ( int i = 0; i < MAX; i++ ) {         
        printf("\n%.2f", (bohnen+i)->gewicht); // (*(bohnen+i)).gewicht
        //Entscheidung Ausgabe Schwach oder Stark
        iStaerke = (int) (bohnen+i)->staerke; // (*(bohnen+i)).staerke
        if ( iStaerke == SCHWACH )
            printf(" / Schwach");
        else if ( iStaerke == STARK )
            printf(" / Stark");
        else
            printf(" / %i", iStaerke);
    }
