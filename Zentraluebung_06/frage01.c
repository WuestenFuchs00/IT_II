...
int main () {
	...
	float fBohnen[iAnzahl];
	float tmp = 0;
 
	//Einlesen der fünf Sorten
	for ( int i = 0; i < iAnzahl; i++ ) { // Gewicht darf nicht 0 oder negativ sein
		//if ( scanf("%i", &fBohnen[i]) == 0  || fBohnen[i] <= 0) {
		if ( scanf("%f", (float *)fBohnen + i) == 0  || fBohnen[i] <= 0) {
			printf("\nUngueltige Eingabe! %f", fBohnen[i]);
			return 1;
		}
	}
	...
	for ( int i = 0; i < iAnzahl-1; i++ ) {
		...
		for ( int j = i+1; j < iAnzahl; j++ ) {
			if ( fBohnen[j] < fBohnen[iMin] ) {
				iMin = j;
			}
		}
		//Tauschen
		tmp = fBohnen[i];
		fBohnen[i] = fBohnen[iMin];
		fBohnen[iMin] = tmp;
	}
